...
// Terminate the program .expect("Can't read Cargo.toml")
// Use a fallback value .unwrap_or("3000".to_string())
// Bubble up the error -> Result<String, reqwest::Error>
// Bubble up multiple errors -> Result<String, Box<dyn std::error::Error>>
// Match boxed errors Err(e) => { if let Some(err) = e.downcast_ref::<reqwest::Error>() { eprintln!("Request Error: {}", err) else { () } }
//
